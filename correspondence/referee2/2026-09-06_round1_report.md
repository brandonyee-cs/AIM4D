...
**Renumbering status (tiers 1-2 complete).** Every manuscript number backed by a tier-1 or tier-2 script has been re-derived from the seeded, Stage-4-corrected pipeline and the auditor's reported values updated to match: 200/200 checks pass with no value absent from the manuscript. Figures 2, 4, 5 and the lead-time figure were regenerated from data (ROC/PR curves with 1,000-resample country-clustered bands on the paper's 29-point grid; reliability bins; permutation bars; lead-time error bars) and spliced into the source, which is the first time any of them has had a code path from CSV to LaTeX. Substantive text changes beyond the channel rewrite: Brier skill now excludes zero ([0.013, 0.335]), so "no demonstrated probabilistic skill" is withdrawn; the permutation-importance ranking moves `network_exposure` from eighth to fourth; the calibration slope is 3.20 [2.25, 4.96]; the closure marginal is -0.231 (ledger) / -0.142 (ERT) with the two design corners now both 0.212 apart. One auditor anchor (C6) had been matching a figure coordinate rather than prose and is repointed. Tier 3 (end-to-end refits, expanding-window CV, seed sweeps, fifteen-episode LOEO) is still running; C1, C5, C27, C28, Figure 3, Appendices I and J, the placebo, and the size-matched arm await it.

**Renumbering status (tier 3 complete, 19:35).** The cascade finished with all 30 scripts exiting 0; `prospective_drivers.py` was excluded by design because the registered forecast is frozen and its amendment is dated. What the long scripts changed, all now in the manuscript and under auditor cover:

- Strict end-to-end rebuild, ledger outcome (eleven origins, 629 rows, 88 positives): AUC 0.669, AP 0.200; end-to-end minus four polyarchy variables -0.017 [-0.117, +0.081]. ERT outcome (535 rows, 109 positives): AUC 0.658, AP 0.293; difference +0.021 [-0.041, +0.088]. The model comparison stays unresolved in both directions, as the power paragraph states.
- Expanding-window cross-validation: AUC-ROC 0.817 [0.73, 0.90], AUC-PR 0.541 [0.42, 0.66] (Table 2).
- Network channel weights across 10 seeds: contiguity 0.24, alliance 0.26, economic similarity 0.23, cultural-linguistic 0.28; the cultural channel is largest in 8 of 10 seeds and in the locked fit (0.282 / 0.260 / 0.243 / 0.215). Appendix I had still carried the pre-fix locked weights (0.273 contiguity-dominant); corrected.
- Network dependence across 10 seeds: United States 0.422 (sd 0.206), Hungary 0.244, Poland 0.132, Turkiye 0.050; pairwise seed rank correlations 0.76 mean, 0.54 minimum (were 0.81 / 0.61); Hungary above Turkiye in 10 of 10; Turkiye positive in 10 of 10 (was 9); Hungary negative in 8 of 10 with seed mean -0.223 against the locked run's +0.419. Locked 2025 cross-section: United States fifteenth of 135 at 0.291 (was twelfth), Serbia 0.462, Hungary 0.415; 44 percent of country-years signed toward autocracy (Section 8, was 45). Table 20 and Figure 3 regenerated from the CSVs. The Appendix C.7 alternative-edge MSE range (31.0 to 53.1 percent) was computed under the earlier Stage 4 specification and the text now says so.
- Contagion shares across seeds (Hungary 0.585, sd 0.109; Turkiye 0.303, sd 0.040) are no longer reported in the manuscript, which withdrew share language; the auditor keeps them as a data check (C14).
- Fifteen-episode full-pipeline LOEO: mean risk delta +0.093 (sd 0.065, largest 0.211), fourteen of fifteen higher, Egypt the exception at -0.008. The meta-only LOEO in the paper remains conservative relative to the full refit, as before (+0.081).

Auditor coverage was extended with C37 (seed sweep, ten checks) and C38 (locked cross-section, six checks) because Section 5.6 had no coverage and its numbers had drifted unnoticed; the auditor now runs 222 checks, 222 PASS. Open items that are author judgment and not renumbering: the hand-typed tables and remaining figures (major concern 3), the length cut, and README plus dependency pinning (Audit 3).
