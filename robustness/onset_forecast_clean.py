...
def main():
    ...
    feats = [c for c in d.columns if c not in EXCLUDE_COLS
             and c not in ("v2x_regime", "onset_year", "ep_end", "in_episode", "at_risk")
    ...
    pd.DataFrame(rows).to_csv(os.path.join(OUT, "onset_forecast_clean.csv"), index=False)
    ci = pd.DataFrame(ci_rows)
    ci.to_csv(os.path.join(OUT, "onset_ablation_ci.csv"), index=False)
    ...
    print("\nWrote onset_forecast_clean.csv and onset_ablation_ci.csv")
